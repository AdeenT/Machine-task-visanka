...
import 'package:ourESchool/core/services/AuthenticationServicesMysql.dart';
import 'package:ourESchool/core/viewmodel/BaseModel.dart';
import 'package:ourESchool/locator.dart';

class LoginPageModel extends BaseModel {
  final _authenticationService = locator<AuthenticationServicesMysql>();
  ...
  Future checkUserDetails({
    ...
    if (response == ReturnType.EMAILERROR) {
      ...
      if (buttonType == ButtonType.LOGIN) {
        AuthErrors res = await _loginUser(email ?? "", password ?? "",
            userType ?? UserType.UNKNOWN, schoolCode ?? "");
        setState(ViewState.Idle);
  ...
  Future _loginUser(String email, String password, UserType userType,
    ...
    AuthErrors authError =
        await _authenticationService.emailPasswordSignInMySQL(
      email,
      password,
      userType,
      schoolCode,
  ...
  Future _registerUser(String email, String password, UserType userType,
      String schoolCode) async {
    AuthErrors authError =
        await _authenticationService.emailPasswordRegisterMySQL(
      email,
      password,
      userType,
      schoolCode,
